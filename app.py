...
# --- FUNÇÃO DE LIMPEZA (Elimina '?' e erros de acentuação) ---
def limpar(texto):
    if not isinstance(texto, str): texto = str(texto)
    # Remove acentos e converte para ASCII para o PDF não quebrar
    return "".join([c for c in unicodedata.normalize('NFKD', texto) if not unicodedata.combining(c)])

# --- FUNÇÃO GERAR LAUDO (Estrutura conforme seu modelo) ---
...
if arquivo:
    ...
    if features and target:
        ...
        # Sidebar - Variaveis com Limites de Extrapolacao
        st.sidebar.header("Parametros")
        inputs = {}
        for f in features:
            f_clean = limpar(f)
            # Define limites baseados no seu modelo (exemplo: Setor Urbano)
