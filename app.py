...
def gerar_laudo_pdf(d, fig, eq_str, inputs, info_extra):
    buffer = io.BytesIO()
...
if arquivo:
    ...
    # Seleção do alvo
    target = st.sidebar.selectbox("Coluna Valor Unitário (Alvo):", cols)
    # Variáveis explicativas (exclui o alvo automaticamente)
    features = st.sidebar.multiselect("Variáveis Explicativas:", [c for c in cols if c != target])
    
    # NOVOS CAMPOS: Inputs de texto para o laudo
    ...
    info_extra = {
        ...
        "Informante/Tel": st.sidebar.text_input("Informante/Telefone")
    }
    ...
    if features and target:
        ...
        if not df_c.empty:
            ...
            st.sidebar.header("⚙️ Parâmetros para Cálculo")
            ...
            if st.sidebar.button("Calcular Precificação"):
                ...
                pdf = gerar_laudo_pdf({'vu': vu, 'min': min_v, 'max': max_v, 'total': total}, fig, eq_str, inputs, info_extra)
