...
def calcular_grau(modelo, n, p_bp, p_sw):
    ...
    # Amplitude relativa média do IC 80%
    pred = modelo.predict(modelo.model.exog)
    amp = ((ci[1] - ci[0]) / (2 * pred)).mean()
...
def gerar_laudo_pdf(grau, amp, p_bp, p_sw):
    ...
    c.drawString(50, 760, f"Classificação: {grau}")
    c.drawString(50, 745, f"Amplitude IC 80%: {amp:.2%}")
    c.drawString(50, 730, f"Teste Breusch-Pagan (p): {p_bp:.4f}")
    c.drawString(50, 715, f"Teste Shapiro-Wilk (p): {p_sw:.4f}")
    c.save()
    buffer.seek(0)
    return buffer
...
if arquivo:
    ...
    if features and target:
        ...
        df_limpo = df[features + [target]].dropna()
        X, y = df_limpo[features], df_limpo[target]
        
        # Cálculo do Modelo Robusto
        modelo, trans, p_bp, p_sw = treinar_modelo_robusto(X, y)
        grau, amp = calcular_grau(modelo, len(df_limpo), p_bp, p_sw)
        
        st.subheader("Resultados Técnicos")
        st.write(f"Classificação Normativa: **{grau}**")
        st.write(f"Status da Homocedasticidade: {'✅ Válido' if p_bp > 0.05 else '❌ Inconsistente'}")
